...
def listar_solicitudes(db: Session = Depends(get_db)):
    return db.query(models.Solicitudes).options(
        joinedload(models.Solicitudes.estatus),
        joinedload(models.Solicitudes.estudiante),
        joinedload(models.Solicitudes.constancia),
        joinedload(models.Solicitudes.trabajador)
    ).all()
...
def eliminar_solicitud(id_solicitud: int, db: Session = Depends(get_db)):
    solicitud = db.query(models.Solicitudes).filter(models.Solicitudes.id == id_solicitud).first()
