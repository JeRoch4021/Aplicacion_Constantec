...
def registrar_solicitud(
    data_constancia: schemas.CrearConstanciaRequest, db: Session = Depends(get_db)
):

    if (
        getattr(data_constancia, "descripcion", None) is None
        or len(data_constancia.descripcion) == 0
    ):
        raise HTTPException(
            status_code=400, detail="No se puede crear una solicitud sin descripcion"
        )

    solicitud = crud_estudiante.crear_solicitud(
        db,
        data_constancia.id_estudiante,
...
@router.post("/estado")
def obtener_estado_constancia(
    data: schemas.SolicitudEstado, db: Session = Depends(get_db)
):
...
def actualizar_estado(
    data: schemas.SolicitudNuevoEstado, db: Session = Depends(get_db)
):
    solicitud = crud_estudiante.actualizar_estado_solicitud(
        db, data.ID_Solicitud, data.Nuevo_Estado
    )
    if not solicitud:
