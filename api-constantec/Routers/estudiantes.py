...
def cambiar_contrasena(
    data: schemas.EstudiantesContrasenaUpdate, db: Session = Depends(get_db)
):
    estudiante_actualizado = crud_estudiante.actualizar_contrasena(
        db, data.No_Control, data.Nueva_Contrasena
    )
...
def buscar_perfil(
    ...
    if estudiante.No_Control != auth_user.get("sub"):
        raise HTTPException(
            detail="No de control no coincide con el estudiante", status_code=404
        )
